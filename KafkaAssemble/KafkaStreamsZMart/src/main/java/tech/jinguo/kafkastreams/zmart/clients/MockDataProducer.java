package tech.jinguo.kafkastreams.zmart.clients;
...
import tech.jinguo.kafkastreams.zmart.model.*;
import tech.jinguo.kafkastreams.zmart.util.datagen.DataGenerator;
import tech.jinguo.kafkastreams.zmart.util.enums.Topics;
...
public class MockDataProducer {
    ...
    // bin\windows\kafka-topics.bat --create --zookeeper localhost:2181,localhost:2182,localhost:2183 --replication-factor 1 --partitions 3 --topic transactions
    ...
    public static void produceStockTransactions(int numberIterations) {
        produceStockTransactions(numberIterations, DataGenerator.NUMBER_TRADED_COMPANIES, DataGenerator.NUMBER_UNIQUE_CUSTOMERS, false);
    }
    ...
    public static void produceStockTransactionsAndDayTradingClickEvents(int numberIterations, int numberTradedCompanies, int numClickEvents, Function<StockTransaction, String> keyFunction) {
        ...
        List<DataGenerator.Customer> customers = DataGenerator.generateCustomers(DataGenerator.NUMBER_UNIQUE_CUSTOMERS);
    ...
    private static void populateCustomersGlobalKTable(List<DataGenerator.Customer> customers) {
        ...
        for (DataGenerator.Customer customer : customers) {
            ...
            ProducerRecord<String, String> record = new ProducerRecord<>(Topics.CLIENTS.topicName(), customer.getCustomerId(), customerName);
            producer.send(record, callback);
        }
    }
    ...
    private static void populateCompaniesGlobalKTable(List<PublicTradedCompany> companies) {
        init();
        for (PublicTradedCompany company : companies) {
            ProducerRecord<String, String> record = new ProducerRecord<>(Topics.COMPANIES.topicName(), company.getSymbol(), company.getName());
            producer.send(record, callback);
        }
    }
    ...
    public static void produceStockTickerData() {
        produceStockTickerData(DataGenerator.NUMBER_TRADED_COMPANIES, DataGenerator.NUM_ITERATIONS);
    }
